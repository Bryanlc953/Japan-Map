...
function ouvrirFiche(prefectureId) {
  ...
  imgDrapeau.src = `../flagss/${prefectureId}.svg`;
...
searchInput.addEventListener('input', e => {
  ...
  if (resultats.length > 0) {
    resultats.forEach(id => {
      ...
      li.innerHTML = `
        <img src="../flagss/${id}.svg" alt="Drapeau ${info.romaji}" class="sugg-flagss">
...
// =========================================================
//   EFFET RIVIÈRE (FRONTIÈRES SVG) AU SCROLL
// =========================================================
window.addEventListener('DOMContentLoaded', () => {
    const lignesFrontieres = document.querySelectorAll('#calque-frontieres path');
    
    // 1. On prépare toutes les lignes (on calcule leur longueur)
    lignesFrontieres.forEach(ligne => {
        const longueur = ligne.getTotalLength();
        ligne.style.setProperty('--longueur-tracé', longueur);
        ligne.style.strokeDasharray = longueur;
    });

    // 2. On crée un "observateur" pour surveiller l'écran
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            // Si la carte entre dans l'écran
        ...
        // 0.3 signifie que l'animation se lance quand 30% de la carte est visible
        threshold: 0.3 
    });

    // 3. On indique à l'observateur quel élément surveiller
    const carteSection = document.querySelector('.app-container');
    if (carteSection) {
        observer.observe(carteSection);
    }
});
