...
function ouvrirFiche(prefectureId) {
  ...
  // Éléments pour le portrait géant
  const portraitGroup = document.getElementById('portraitPathGroup');
  const citiesGroup = document.getElementById('portraitCitiesGroup');
  const svgPortrait = document.getElementById('svgPortrait');

  // On cible le tracé original via l'attribut data-prefecture
  const originalPath = document.querySelector(`.prefecture[data-prefecture="${prefectureId}"]`);

  // --- LOGIQUE D'AGRANDISSEMENT MAXIMUM ---
  if (originalPath && portraitGroup && svgPortrait) {
    portraitGroup.innerHTML = "";
    citiesGroup.innerHTML = "";

    // Clonage de la forme
    const clonedPath = originalPath.cloneNode(true);
    clonedPath.removeAttribute("id");
    clonedPath.setAttribute("class", "portrait-shape");
    portraitGroup.appendChild(clonedPath);

    // Calcul des dimensions réelles pour le zoom
    const box = originalPath.getBBox();
    
    // Marge minuscule (2%) pour que ça prenne TOUTE la place
    const margin = 0.02; 
    const viewW = box.width * (1 + margin * 2);
    const viewH = box.height * (1 + margin * 2);

    // On cadre la "caméra" pile sur la préfecture
    svgPortrait.setAttribute("viewBox", `${box.x - (box.width * margin)} ${box.y - (box.height * margin)} ${viewW} ${viewH}`);

    // Épaisseur du trait harmonisée (très fin car zoomé)
    const diagonal = Math.sqrt(viewW ** 2 + viewH ** 2);
    const visualStroke = diagonal / 350; 
    clonedPath.style.strokeWidth = `${visualStroke}px`;

    // Ajout de la capitale (point rouge)
    if (info.capitale) {
        const dot = document.createElementNS("http://www.w3.org/2000/svg", "circle");
        dot.setAttribute("cx", box.x + box.width / 2);
        dot.setAttribute("cy", box.y + box.height / 2);
        dot.setAttribute("r", diagonal / 100); /*taille du point*/
        dot.setAttribute("class", "city-marker");
  ...
  // 3. Remplissage des textes et animation
